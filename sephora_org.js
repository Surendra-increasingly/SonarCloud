...
(function () {
    ...
    INC.config.addToBasketURL = "https://uat.sephora.co.uk/shop/buy";
    INC.clientConfig.checkoutLink = "https://uat.sephora.co.uk/shop/basket";
    ...
    INC.config.noimage = "https://www.increasingly.co/Clients/flUN19e/sephora/images/no-image.png"
    // ------------------------
    INC.config.recommendation = false;
    INC.config.bundledid = []
    INC.config.cldomain = "";
    INC.config.checkoutpath =  '/shop/basket';
    ...
    function getCountry() {
        let country = ""
        try {
            if (document.querySelector('#select_country_footer i').getAttribute("data-flag-img").split("country-flags/")[1] == "gb.png") {
                country = true
            } else {
                country = false
            }
    ...
    INC.init = function () {
        ...
        switch (INC.config.pageType) {
            case "pdp":
                ...
                let oos_element = document.querySelector('.oos-subsku-name')
                if (oos_element) {
    ...
    function cartpageprodids() {
        ...
        Array.prototype.forEach.call(document.querySelectorAll('#mybasket .bagItem'), function (elProductCode) {
            let cartId = JSON.parse(elProductCode.querySelector('.bagItem-utils a').getAttribute('data-feelunique-datalayer-push'))
            if (cartId != null) {
                let cart_id = cartId.remove.ecommerce.remove.products[0].id
                INC.config.cartproductids.push(cart_id);
            }
        });
                    ...
                    if (INC.config.dupDataObjectProducts.indexOf(bundles[i].BundleId) == -1) {
                        ...
                        for (let ir in irbProductsArray) {
                            if (Object.prototype.hasOwnProperty.call(irbProductsArray, ir)) {
                                ...
                                if (isBase64(product["Description"]) == trueflag) {
                                    product["Description"] = Base64.decode(product["Description"]).trim()
                                } else {
                                    try {
                                        if (isBase64(product["Description"]) == falseflag && product["Description"].indexOf("CAgICAgICAgI") > -1) {
                                            product["Description"] = Base64.decode(product["Description"]).trim()
                                        }
                                    ...
                                    if (pAttributes != "" && pAttributes != null) {
                                        for (let p of pAttributes) {
                                            ...
                                            if (pAttributes.length == 1) {
                                                if (p.attributeCode == "color" || p.attributeCode == "Color" || p.attributeCode == "COLOUR" || p.attributeCode == "Colour") {
                                                    p.attributeCode = "Color"
                                                } else {
                                                    p.attributeCode = "Size"
                                                }
    ...
    INC.methods.irbReq = function (productId, bundleCount, elementName) {
        ...
        if (window.location.href == "https://uat.sephora.co.uk/p/ICONIC-London-Illuminator-Drops-135ml" || window.location.href == "https://uat.sephora.co.uk/p/Benefit-Hello-Happy-Velvet-Powder-Foundation-7g" || window.location.href == "https://uat.sephora.co.uk/p/ELEMIS-Superfood-Midnight-Facial-50ml") {
                                        ...
                                        if (INC.config.empty_prod_details[0] != undefined) {
                                            if (INC.config.empty_prod_details[0].p_activePrice != undefined) {
                                                if (subtotalinc_active != null) {
                                                    ...
                                                    if (INC.config.empty_prod_details[0].regularPrice != undefined) {
                                                        ...
                                                        if (totalsaveprc != 0){
                                                            ...
                                                            document.querySelector('.inc_cart_added_product_desc_ecirp_block').classList.add("is_special_ecirp");
                                                        }
                                                    } else {
                                                        subtotalinc_active.innerHTML = formatter.format(subt)
                                                        subtotalinc_active.setAttribute('subtotalactiveprice', subt)
                                                ...
                                                if (INC.config.empty_prod_details[0].p_name != undefined) {
                                                    prd_names.innerHTML = INC.dataStore.plpProductQty + "x" + INC.config.empty_prod_details[0].p_name
                                                }
                                                if (INC.config.empty_prod_details[0].p_name != undefined) {
                                                ...
                                                if(INC.config.empty_prod_details[0].regularPrice != INC.config.empty_prod_details[0].p_activePrice){
                                                    let prd_regprice = document.querySelector('.inc_cart_added_product_desc_ecirp_regular_text')
                                                    if (prd_regprice != null && INC.config.empty_prod_details[0].regularPrice != null) {
                                                        prd_regprice.innerHTML = formatter.format(INC.config.empty_prod_details[0].regularPrice);
                                                    }
                                                }
                                    ...
                                    jQuery('#subbuyform #buysub').on("touchstart click", function (ev) {
                                        if (this.parentNode.querySelector('#sp_opt1_title') != null) {
                                            if (this.parentNode.querySelector('#sp_opt1_title').innerText.indexOf('Please select') == -1) {
                                                ...
                                                var colorText = this.parentNode.querySelector('#sp_opt1_title').innerText.split('-')[0]
                                                var colorImg = this.parentNode.querySelector('#sp_opt1_title img').src
                                                document.querySelector('.inc_cart_added_product_desc_att_size_text').innerHTML = "<img class='inc_lazy' src='"+ colorImg +"' data-src='"+ colorImg +"'></>" + "<span>" + colorText + "</span>"
                                            }
                                        }
                                    })
                                },1400)
    ...
    INC.methods.getProductIdFromWebPage = function () {
        let PRODUCTID = ""
        if (document.querySelector('#buy-this-product input[name="p"]') != null) {
            PRODUCTID = document.querySelector('#buy-this-product input[name="p"]').value.trim();
        }
    ...
    INC.methods.showSidebar = function () {
        ...
        if (sidebarel != null) {
            if (document.querySelector('.inc_recommended_tabs_list_item_block') == null || INC.config.sidebarFirstLoad == falseflag) {
                setTimeout(() => {
                    window.location.replace('https://uat.sephora.co.uk/shop/basket');
                }, 1000);
                return
            }
            setTimeout(() => {
                if(document.querySelector('#simplemodal-container') != null){
                    if(document.querySelector('.modalCloseImg.simplemodal-close') != null){
                        document.querySelector('.modalCloseImg.simplemodal-close').click()
                    }
                }
            },500);
    ...
    INC.methods.createAMBlock = function (PDP_type) {
        if (INC.dataStore.dataStoreObj.undefined == undefined) {
            ...
            let elAMPosition = document.querySelector('#white-background');

            if (PDP_type == "PDP") {
                if (document.querySelector('#chanel-strip-banner') != null) {
                    let pdpBlockSibling = document.querySelector('.container.white-boxes');
                    pdpBlockSibling.parentNode.insertBefore(amBlock, pdpBlockSibling);

                } else if( document.querySelector('#product-description') != null){
                    let pdpBlockSibling = document.querySelector('#product-description');
                    pdpBlockSibling.parentNode.insertBefore(amBlock, pdpBlockSibling);
                }else if( document.querySelector('.zone-description') != null){
                    let pdpBlockSibling = document.querySelector('.zone-description')
                    pdpBlockSibling.parentNode.insertBefore(amBlock, pdpBlockSibling);
                }
            } else {
                if (bundleProductsArray.length > 0) {
                    let elAMPositioncart = elAMPosition.nextElementSibling
                    let amBlockSelector = elAMPosition;
        ...
        if (sidebar != "sidebar" && sidebar != "hidesidebar") {
            if (INC.dataStore.dataStoreObj.undefined != undefined || empty_sidebarflag == falseflag) {
                ...
                if(pdpBlock.querySelector('.inc_pdp_product-main_block .inc_product_desc_ecirp_active_text') != null){
                    let actprc = pdpBlock.querySelector('.inc_pdp_product-main_block .inc_product_desc_ecirp_active_text').innerText.replace('£','').replace(',','')
                    let regprc = pdpBlock.querySelector('.inc_pdp_product-main_block .inc_product_desc_ecirp_regular_text').innerText.replace('£','').replace(',','')
                    let prdi_d = pdpBlock.querySelector('.inc_pdp_product-main_block .inc_product_module_block').getAttribute('data-id')
                    let clientPrice = null
                    let clientSpPrice =null
                    let price_block = document.querySelector('.price-info')
                    
                    if(price_block.querySelector('.Price') != null){
                        clientPrice = price_block.querySelector('.Price').innerText.split('£')[1].trim().replace(',', '')
                    }
                    if(price_block.querySelector('.rrp') !== null){
                        clientPrice = price_block.querySelector('.rrp').innerText.split('£')[1].trim().replace(',', '')
                        clientSpPrice = price_block.querySelector('.Price').innerText.split('£')[1].trim().replace(',', '')
                    }
                    if(document.querySelector('#plsSelect .priceStock span') != null){
                        clientPrice = document.querySelector('#plsSelect .priceStock span').innerText.split('£')[1].trim().replace(',', '')
                        clientSpPrice = null
                    }
                    
                    let pricecheck_ = clientPrice
                    if(clientSpPrice != null){
                        pricecheck_ = clientSpPrice
                    }
                    
                    let incpricecheck_ = actprc
                    
                    // if(parseFloat(incpricecheck_) != parseFloat(pricecheck_)){
                    //     if(clientPrice == 0 || clientPrice == null){
                    //         return
                    //     }
                    //     if(regprc == ""){
                ...
                let client_var_size = ""
                let client_var = ""
                if (document.querySelector('.shadeSelector #plsSelect .details .name') != null) {
                    client_var = document.querySelector('.shadeSelector #plsSelect .details .name').innerText.trim().toLowerCase();
                }
            ...
            bundleCartProductsArray.forEach(function (productSKU) {
                ...
                if (INC.clientConfig.OOS.indexOf(productObj.sku) >= 0) {
                    ...
                    if (color != "" && color != undefined && color != null) {
                        sel_size = "<img class='inc_lazy' src='"+ colorImg +"' data-src='"+ colorImg +"'></>" + "<span>" + color + "</span>"
                    }
                    ...
                    let regularPrice = parseFloat(productObj.regularPrice).toFixed(2);
    ...
    INC.methods.addProductToSidebarCart = function (findObj, client_add_btn, _productBlockel, btn_addtocart, elmparent) {
        ...
        if (client_add_btn == 'client_add_btn') {
            if (INC.config.pageType == "pdp"){
                if (elmparent.querySelector('.product-detail-information select') != null){
                    qty = elmparent.querySelector('.product-detail-information select').value;
                }
            } else {
        ...
        if (sbBlock != null) {
            ...
            if (INC.clientConfig.OOS.indexOf(productObj.mainId) >= 0) {
                ...
                if(price_block != null){
                    if (price_block.querySelector('.Price').innerText != "") {
                        pdpprice = price_block.querySelector('.Price').innerText.split('£')[1].trim().replace(',', '')
                    }
                }
                if (elmparent != null) {
    ...
    function prepareqty(productBlock, blockType,elPackOfListBlock,mainprod) {
        ...
        let elprdqtyinputblock = productBlock.querySelector('.inc_product_qty_input_block');
        let Mainqty_c = 5
        if (document.querySelectorAll('select[name="quantity"] option') != null && blockType == "pdp" && productBlock.getAttribute('data-main_id') == INC.methods.getProductIdFromWebPage()) {
            if (document.querySelectorAll('select[name="quantity"] option').length != 0) {
                Mainqty_c = document.querySelectorAll('select[name="quantity"] option').length
            } else if (document.querySelectorAll('ul[role="listbox"] li')[1] != undefined) {
                Mainqty_c = document.querySelectorAll('ul[role="listbox"] li').length
            }
        }
        ...
        if(mainprod == "mainprod"){
            elQtyInput.setAttribute('max', Mainqty_c);
            elQtyInput.setAttribute('maxlength', 1);
        }else{
            elQtyInput.setAttribute('max', 5);
            elQtyInput.setAttribute('maxlength', 1);
        }
        elQtyInput.value = 1;
        ...
        elprdqtyinputblock.addEventListener("click", function () {
            ...
            modalProductBlock.querySelector('.inc_product_desc_att_color_block').classList.remove('show_dropdown');
        });
        ...
        for (let i = 0; i < qty_c; i++) elPackOfArrray.push(i);
        elPackOfArrray.forEach(function (i) {
            let elPackOfItemBlock1 = generate_html_tag('div', 'inc_product_desc_pack_of_item_block');
            let elPackOfTitleText = generate_html_tag('div', 'inc_product_desc_pack_of_item_title_text');
            elPackOfTitleText.innerText = "";
            let elPackOfItemqtyBlock = generate_html_tag('div', 'inc_product_desc_pack_of_item_quantity_text');
            elPackOfItemqtyBlock.innerText = i + 1;
            elPackOfItemBlock1.appendChild(elPackOfTitleText)
            elPackOfItemBlock1.appendChild(elPackOfItemqtyBlock)
            ...
            elPackOfItemBlock1.addEventListener("click", function () {
                ...
                showcaseProductBlock.querySelector('.inc_product_qty_block').querySelector('input').value = qty;
                if(qty > 1){
                    showcaseProductBlock.querySelector('.inc_product_desc_qty_add_btn_block .inc_product_qty_block').classList.remove("singleqty")
                }else if(qty == 1){
                    showcaseProductBlock.querySelector('.inc_product_desc_qty_add_btn_block .inc_product_qty_block').classList.add("singleqty")
                }
                showcaseProductBlock.querySelector('.inc_mobile_qty_block').querySelector('input').value = qty;
        ...
        function qtyupdateInc(_elm, prdblk, eladtext) {
            ...
            if(mainprod == "mainprod"){
                if (_elm.value > parseInt(Mainqty_c)) {
                    _elm.value = parseInt(Mainqty_c);
                }
                if (_elm.value == parseInt(Mainqty_c)) {
            ...
            if (prdblk.querySelector('.inc_product_desc_pack_of_item_block.active') != null) {
                prdblk.querySelector('.inc_product_desc_pack_of_item_block.active').classList.remove('active')
            }
            if (prdblk.querySelector('.inc_product_desc_pack_of_item_block[index_val="' + qty + '"]') != null) {
                prdblk.querySelector('.inc_product_desc_pack_of_item_block[index_val="' + qty + '"]').classList.add('active')
            }
        }
        ...
        elQtyDownBlock.addEventListener('click', function () {
            ...
            if(mainprod == "mainprod"){
                if(parseInt(elinput.value) <= parseInt(Mainqty_c)){
                    elQtyUpBlock.classList.remove('maxqty')
                }
            }else{
                if(parseInt(elinput.value) <= 5){
                    elQtyUpBlock.classList.remove('maxqty')
                }
            }
        ...
        elQtyUpBlock.addEventListener('click', function () {
            ...
            if(mainprod == "mainprod"){
                elinpt.value = (parseInt(elinpt.value) + 1 > parseInt(Mainqty_c)) ? parseInt(Mainqty_c) : parseInt(elinpt.value) + 1;
                if(parseInt(elinpt.value) == parseInt(Mainqty_c)){
                    elQtyUpBlock.classList.add('maxqty')
                }
            }else{
                elinpt.value = (parseInt(elinpt.value) + 1 > 5) ? 5 : parseInt(elinpt.value) + 1;
                if(parseInt(elinpt.value) == 5){
                    elQtyUpBlock.classList.add('maxqty')
                }
            }
            let productModuleBlock = productBlock.parentNode.parentNode;
    ...
    INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock, elProductShowcaseBlock, mainprod) {
        ...
        let elAttClonedColorTitleText = productBlock.querySelector('.inc_product_desc_att_cloned_color_title_text');
        let elAttClonedColorListBlock = productBlock.querySelector('.inc_product_desc_att_cloned_color_list_block');
        ...
        let elPackOfListBlock = productBlock.querySelector('.inc_product_desc_pack_of_list_block');
        let elmobileQtyInputBlock= productBlock.querySelector('.inc_mobile_qty_input_block');
        let elMobilePackOfListBlock = productBlock.querySelector('.inc_mobile_product_desc_pack_of_list_block');
        let elshowColorCount = productBlock.querySelector('.inc_show_color_count');
        let elincsavePer = productBlock.querySelector('.incsavePer');
        ...
        if (productObj.PromotionalMessage != null && productObj.PromotionalMessage != "" && productObj.PromotionalMessage != "null") {
        ...
        let totalsaveprc = get_discount_price_percentage(productObj.regularPrice, productObj.activePrice)
        ...
        if (activePrice != regularPrice && regularPrice != "" && regularPrice != null && regularPrice != 0) {
            ...
            elproductPriceLabel.innerHTML = "-"+totalsaveprc+"%"
            if(totalsaveprc > 0){
                elincsavePer.classList.add("active")
                elincsavePer.innerHTML = "-"+totalsaveprc+"%"
            }
        } else {
            elPriceActiveText.innerHTML = formatter.format(activePrice)
        ...
        prepareqty(productBlock, blockType,elPackOfListBlock,mainprod)

        //Mobile mini popup Qty
        let elQtyMobBlock = productBlock.querySelector('.inc_mobile_qty_block');
        elQtyMobBlock.classList.add("singleqty")
        let elQtyMobDownBlock = productBlock.querySelector('.inc_mobile_qty_down_block');
        let elQtyMobUpBlock = productBlock.querySelector('.inc_mobile_qty_up_block');
        let elmobileprdqtyinputblock = productBlock.querySelector('.inc_mobile_qty_input_block');
        ...
        elQtyInputMob.setAttribute("disabled", "");
        ...
        elmobileprdqtyinputblock.addEventListener("click", function () {
            ...
            elMobilePackOfListBlock.classList.toggle("active");
        ...
        if (document.querySelectorAll('select[name="quantity"] option') != null && blockType == "pdp" && productBlock.getAttribute('data-main_id') == INC.methods.getProductIdFromWebPage()) {
            if (document.querySelectorAll('select[name="quantity"] option').length != 0) {
                qty_c1 = document.querySelectorAll('select[name="quantity"] option').length
            } else if (document.querySelectorAll('ul[role="listbox"] li')[1] != undefined) {
                qty_c1 = document.querySelectorAll('ul[role="listbox"] li').length
            }
            if (qty_c1 == 0) {
                qty_c1 = 5
            }
        }
        for (let i = 0; i < qty_c1; i++) elPackOfArrray1.push(i);
        elPackOfArrray1.forEach(function (i) {
            let elMobilePackOfItemBlock = generate_html_tag('div', 'inc_mobile_product_desc_pack_of_item_block');
            let elMobilePackOfTitleText = generate_html_tag('div', 'inc_mobile_product_desc_pack_of_item_title_text');
            elMobilePackOfTitleText.innerText = "";
            let elMobilePackOfItemqtyBlock = generate_html_tag('div', 'inc_mobile_product_desc_pack_of_item_quantity_text');
            elMobilePackOfItemqtyBlock.innerText = i + 1;
            elMobilePackOfItemBlock.appendChild(elMobilePackOfTitleText)
            elMobilePackOfItemBlock.appendChild(elMobilePackOfItemqtyBlock)
            elMobilePackOfListBlock.appendChild(elMobilePackOfItemBlock);

            elMobilePackOfItemBlock.addEventListener("click", function () {
                elMobilePackOfListBlock.classList.remove("active");
                elQtyInputMob.value = i + 1;
                let sku = productBlock.getAttribute('data-sku');
                let qty = parseInt(elQtyInputMob.value);
                let productModuleBlock = productBlock.parentNode.parentNode;
                let showcaseProductBlock = productModuleBlock.querySelector('.inc_product_showcase_block').querySelector('.inc_product_block');
                let modalProductBlock = productModuleBlock.querySelector('.inc_product_modal_block').querySelector('.inc_product_block')
                showcaseProductBlock.querySelector('.inc_product_qty_block').querySelector('input').value = qty;
                modalProductBlock.querySelector('.inc_product_qty_input_block').querySelector('input').value = qty;
                if (elAddText.innerText == "Added" || elAddText.innerText == "Update") {
                    INC.methods.updateBundleCart({
                        sku: sku,
                        qty: qty
        ...
        function qtyupdateInc1(_elm, prdblk, eladtext) {
            ...
            if (prdblk.querySelector('.inc_mobile_product_desc_pack_of_item_block.active') != null) {
                prdblk.querySelector('.inc_mobile_product_desc_pack_of_item_block.active').classList.remove('active')
            }
            if (prdblk.querySelector('.inc_mobile_product_desc_pack_of_item_block[index_val="' + qty + '"]') != null) {
                prdblk.querySelector('.inc_mobile_product_desc_pack_of_item_block[index_val="' + qty + '"]').classList.add('active')
            }
        }

        elmobileQtyInputBlock.appendChild(elQtyInputMob);

        elQtyMobDownBlock.addEventListener('click', function () {
            let elinput = this.parentNode.querySelector('.inc_mobile_qty_input_block input')
            let productModuleBlock = productBlock.parentNode.parentNode;
            let showcaseProductBlock = productModuleBlock.querySelector('.inc_product_showcase_block').querySelector('.inc_product_block');
            let modalProductBlock = productModuleBlock.querySelector('.inc_product_modal_block').querySelector('.inc_product_block')
            elinput.value = (elinput.value - 1 < 1) ? 1 : elinput.value - 1;
        ...
        elQtyMobUpBlock.addEventListener('click', function () {
            let elinpt = this.parentNode.querySelector('.inc_mobile_qty_input_block input')
        ...
        productcolorattcloned(productColorsObj, productBlock, elAttClonedColorListBlock, blockType, elAttColorBlock, elAttClonedColorTitleText,null)
        ...
        if (productColorsObj["colorArray"].length > 0) {
            ...
            productBlock.querySelector('.inc_product_desc_att_cloned_color_title_block').addEventListener('click', function () {
                this.parentNode.classList.toggle('show_dropdown')
                this.parentNode.parentNode.parentNode.parentNode.querySelector('.inc_product_desc_pack_of_list_block').classList.remove("active")
                this.parentNode.parentNode.parentNode.parentNode.querySelector('.inc_qty_addBtn_block .inc_mobile_product_desc_pack_of_list_block').classList.remove("active")
            })
            if(blockType == "am"){
                elshowColorCount.classList.add("active");
                elshowColorCount.innerHTML = ((productColorsObj["colorArray"].length == 1) ? productColorsObj["colorArray"].length + " colour" : productColorsObj["colorArray"].length + " colours");

            }else if(blockType == "sidebar"){
                elshowColorCount.classList.add("active");
                elshowColorCount.innerHTML = ((productColorsObj["colorArray"].length == 1) ? productColorsObj["colorArray"].length + " option" : productColorsObj["colorArray"].length + " options");
            }
        }
        ...
        if (productColorsObj["colorArray"].length == 0 && sizezero["att0Array"].length == 0) {
            ...
            eladdvarianttext.textContent = "Select Shade";
                    ...
                    if(event.target.classList.contains('inc_product_desc_add_img')){
                        ...
                        if (prodflag == trueflag) {
                            ...
                            if(productModuleBlock.querySelector('.inc_product_showcase_block .inc_product_desc_att_details_block .inc_product_desc_att_color_block .inc_product_desc_att_color_list_block .inc_product_desc_att_color_list_item_block.inc_active') != null){
                                if(productModuleBlock.querySelector('.inc_product_showcase_block .inc_product_desc_att_details_block .inc_product_desc_att_color_block .inc_product_desc_att_color_list_block .inc_product_desc_att_color_list_item_block.inc_active .inc_product_desc_att_color_list_item_img img') != null){
                                ...
                                if(productModuleBlock.querySelector('.inc_product_showcase_block .inc_product_desc_att_details_block .inc_product_desc_att_zero_block .inc_product_desc_att_zero_list_block .inc_product_desc_att_size_list_item_img_block.inc_active') != null){
                                    if(productModuleBlock.querySelector('.inc_product_showcase_block .inc_product_desc_att_details_block .inc_product_desc_att_zero_block .inc_product_desc_att_zero_list_block .inc_product_desc_att_size_list_item_img_block.inc_active img') != null){
                                        let VariantImgSrc = productModuleBlock.querySelector('.inc_product_showcase_block .inc_product_desc_att_details_block .inc_product_desc_att_zero_block .inc_product_desc_att_zero_list_block .inc_product_desc_att_size_list_item_img_block.inc_active img').src
                                        variant_img.setAttribute('src', VariantImgSrc)
                                    }
                                }
                            }
    ...
    INC.methods.createProductModalBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock, elmodalblock, mainprod,rec_type) {
        ...
        let elAttClonedColorTitleText = prdmblock.querySelector('.inc_product_desc_att_cloned_color_title_text');
        ...
        let elAttClonedColorListBlock = prdmblock.querySelector('.inc_product_desc_att_cloned_color_list_block');
        ...
        let elratingsmobileBlock = prdmblock.querySelector('.inc_product_mobile_desc_ratings_block');
        ...
        let elproductmanufacturerblobk = prdmblock.querySelector('.inc_product_header_manufacturer_block')
        let elproductdescmanufacturerblobk = prdmblock.querySelector('.inc_product_desc_manufacturer_block')
        let elPackOfListBlock = productBlock.querySelector('.inc_product_desc_pack_of_list_block');
        if(window.innerWidth < 820){
        ...
        elHeader.querySelector('.inc_product_header_img_block').addEventListener("click", function () {
            ...
            productBlock.parentNode.querySelector('.inc_product_desc_pack_of_list_block').classList.remove('active');
            productBlock.parentNode.querySelector('.inc_product_desc_att_color_block').classList.remove('show_dropdown');
            if (inc_scroll_height > 100) {
                window.scrollTo(0, inc_scroll_height);
            }
        });
        ...
        if(productObj.manufacturer != undefined){
            elproductmanufacturerblobk.innerText = productObj.manufacturer;
            if(INC.config.deviceType == "mobile"){
                elproductdescmanufacturerblobk.innerText = productObj.manufacturer;
            }
        } 
        let field1 = productObj.ratingsCount
        let field2 = productObj.reviewsCount
        INC.methods.get_product_ratings(field1, field2, elratingsBlock);
        if(window.innerWidth < 820){
            INC.methods.get_product_ratings(field1, field2, elratingsmobileBlock);
        }
        ...
        elAttSizeListBlock.innerHTML = "";
        elAttColorListBlock.innerHTML = "";
        elAttClonedColorListBlock.innerHTML = "";
        elAtt3ListBlock.innerHTML = "";
        ...
        elAddBlock.addEventListener("click", function () {
            ...
            if(productBlock.parentNode.querySelector('.inc_product_desc_pack_of_list_block') != null){
                productBlock.parentNode.querySelector('.inc_product_desc_pack_of_list_block').classList.remove('active');   
            }
            let productModuleBlock = productBlock.parentNode.parentNode;
            let flag_check = INC.methods.checkVariantSelection(productModuleBlock)
    ...
    function productcoloratt(productColorsObj, productBlock, elAttColorListBlock, blockType, elAttColorBlock, elAttColorTitleText,bType) {
        ...
        if (productColorsObj.colorArray.length > 0) {
            ...
            if (productColorsObj["colorArray"].length > 0) {
                ...
                elAttColorTitleText.innerText = "Colours: "
    ...
    function productcolorattcloned(productColorsObj, productBlock, elAttClonedColorListBlock, blockType, elAttColorBlock, elAttClonedColorTitleText,bType) {
        fabricprod = []
        if (productColorsObj.colorArray.length > 0) {
            productColorsObj["colorArray"] = productColorsObj["colorArray"].sort(function (a, b) {
                if (a.attLebel < b.attLebel) {
            ...
            if (productColorsObj["colorArray"].length > 0) {
                elAttClonedColorListBlock.innerHTML = "";
                elAttColorBlock.classList.add("inc_active");
                elAttClonedColorTitleText.innerText = productColorsObj["colorArray"][0].label.toLowerCase()
                let one_attr_flag = false
                if (productColorsObj["colorArray"].length == 1) {
                    one_attr_flag = true
                }
                if (productColorsObj["colorArray"].length > 6) {
                    elAttClonedColorListBlock.classList.add('greater6');
                }
                let pushedfabric = []
                let colorC = 0
                elAttColorBlock.classList.add("inc_active");
                    ...
                    for (let aL = 0; aL < productColorsObj["colorArray"][0].attrLength; aL++) {
                        productColorsObj["colorArray"].forEach(function (colorObj) {
                            ...
                            if (pushedfabric[aL] == colorObj.attLebel) {
                                ...
                                colorelmObcloned(colorObj, elAttClonedColorListBlock, colorC, blockType, productBlock, one_attr_flag,bType)
                                if (colorObj.label == 'Select Fabric') {
                                    elAttClonedColorTitleText.innerText = colorObj.label
                                }
                                colorC++
                            }
                        });
    ...
    function colorelmObcloned(colorObj, elAttClonedColorListBlock, colorC, blockType, productBlock, one_attr_flag,bType) {
        let attColorBlock = generate_html_tag('div', 'inc_product_desc_att_cloned_color_list_item_block')
        let attColorImgBlock = generate_html_tag('div', 'inc_product_desc_att_cloned_color_list_item_img_block');
        let attColorImg = generate_html_tag('div', 'inc_product_desc_att_cloned_color_list_item_img');
        ...
        if (bType != null && colorC > 5 && blockType != 'pdp') {
            ...
            if (elAttClonedColorListBlock.querySelector('.inc_more_color') == null) {
                elAttClonedColorListBlock.appendChild(inc_more_color);
            }
            return
        }
        attColorImgBlock.appendChild(attColorImg);
        attColorBlock.appendChild(attColorImgBlock);
        elAttClonedColorListBlock.appendChild(attColorBlock);
    ...
    INC.methods.mappvariants = function (obj, blocktype, type_of_variant) {
        if (obj.parentNode != undefined) {
            ...
            if (color_text != undefined) {
                ...
                productblockmod.querySelector('.inc_product_desc_att_cloned_color_title_text_selected').setAttribute('style', 'color:"#4D4D4F";')
                productblockmod.querySelector('.inc_product_desc_att_cloned_color_title_text_selected').innerText = color_text
                productblockshow.querySelector('.inc_product_desc_att_cloned_color_title_text_selected').setAttribute('style', 'color:"#4D4D4F";')
                productblockshow.querySelector('.inc_product_desc_att_cloned_color_title_text_selected').innerText = color_text
            ...
            let elselcolortextmodalclone = productblockmod.querySelector('.inc_product_desc_att_cloned_color_title_text_selected')
            let elselcolortextshowclone = productblockshow.querySelector('.inc_product_desc_att_cloned_color_title_text_selected')
            ...
            let elAttClonedColorListBlock = productblockmod.querySelector('.inc_product_desc_att_cloned_color_list_block')
            ...
            let elclonedcolortextmodal=productblockmod.querySelector('.inc_product_desc_att_cloned_color_title_text')
            ...
            if (type_of_variant == "size" || type_of_variant == "third") {
                ...
                if (productModuleBlck.querySelector('.inc_product_modal_block .inc_product_desc_att_color_title_text_selected').innerText == "" && productModuleBlck.querySelector('.inc_product_desc_att_color_title_text_selected').innerText == "") {
                    ...
                    productModuleBlck.querySelector('.inc_product_modal_block .inc_product_desc_att_cloned_color_title_text_selected').innerText = ""
                    productModuleBlck.querySelector('.inc_product_desc_att_cloned_color_title_text_selected').innerText = ""
                }
                ...
                if (productColorsObj["colorArray"].length > 0) {
                    ...
                    productColorsObj["colorArray"].forEach(function (colorObj) {
                        ...
                        let attClonedColorBlock = generate_html_tag('div', 'inc_product_desc_att_cloned_color_list_item_block')
                        let attClonedColorImgBlock = generate_html_tag('div', 'inc_product_desc_att_cloned_color_list_item_img_block');
                        let attClonedColorImg = generate_html_tag('div', 'inc_product_desc_att_cloned_color_list_item_img');
                        ...
                        attClonedColorImgBlock.setAttribute('blocktype', blocktype)
                        attClonedColorImgBlock.setAttribute('varianttypes', 'color')
                        attClonedColorImgBlock.addEventListener('click',function(){
                            let elvariantc2 = this;
                            let elblktypec2=this.getAttribute('blocktype')
                            let elvtypec2=this.getAttribute('varianttypes')
                            INC.methods.mappvariants(elvariantc2,elblktypec2,elvtypec2)
                        ...
                        imgcolorgen(colorObj,attColorImg)
                        imgcolorgen(colorObj,attClonedColorImg)
                        let colorTitle = generate_html_tag('a');
                        colorTitle.innerText = colorObj["text"]
                        ...
                        let colorTitle1 = generate_html_tag('a');
                        colorTitle1.innerText = colorObj["text"]
                        colorTitle1.title = colorObj["text"];
                        colorTitle1.addEventListener('click', function (e) { e.preventDefault(); })
                        attClonedColorImg.appendChild(colorTitle1);
                        ...
                        attClonedColorImgBlock.setAttribute('data-color', colorObj["text"]);
                        attClonedColorImgBlock.setAttribute("datacolor", colorObj["text"].toLocaleLowerCase());
                        attClonedColorImgBlock.setAttribute('data-option', "0");
                        attClonedColorImgBlock.setAttribute('data-cpid', colorObj["cpid"])
                        attClonedColorImgBlock.setAttribute('data-colorcode', colorObj["colorCode"])
                        attClonedColorImgBlock.setAttribute('data-optionimge', colorObj["imgSrc"])
                        attClonedColorImgBlock.appendChild(attClonedColorImg);
                        attClonedColorBlock.appendChild(attClonedColorImgBlock);
                        ...
                        let attColorBlockclone1 = attClonedColorBlock.cloneNode(attClonedColorBlock);
                        attColorBlockclone1.querySelector('.inc_product_desc_att_cloned_color_list_item_img_block').addEventListener('click',function(){
                            let elvariantc1s = this;
                            let elblktypec2s=this.getAttribute('blocktype')
                            let elvtypec2s=this.getAttribute('varianttypes')
                            INC.methods.mappvariants(elvariantc1s,elblktypec2s,elvtypec2s)
                        ...
                        clrarrac++
                        elAttColorListBlock.appendChild(attColorBlock);
                        elAttClonedColorListBlock.appendChild(attClonedColorBlock);
                        ...
                        if (color_text != null){
                            if(color_text.toLocaleLowerCase() == colorObj.text.toLocaleLowerCase()){
                                ...
                                attClonedColorBlock.classList.add('inc_active');
                                attColorBlockclone1.classList.add('inc_active');
                                elselcolortextshow.innerText= colorObj.text
                                elselcolortextmodal.innerText= colorObj.text + ":"
                                elselcolortextmodalclone.innerText= colorObj.text + ":"
                                elselcolortextshowclone.innerText= colorObj.text + ":"
                                if(window.innerWidth < 821){
                                    elcolortextmodal.style.backgroundColor = colorObj["colorCode"];
                                    elclonedcolortextmodal.style.backgroundColor = colorObj["colorCode"];
                ...
                if(productblockmod.querySelector('.inc_product_desc_att_color_block.show_dropdown') != null){
                    productblockmod.querySelector('.inc_product_desc_att_color_block.show_dropdown').classList.remove('show_dropdown')
                }
                if(productblockshow.querySelector('.inc_product_desc_att_color_block.show_dropdown') != null){
                    productblockshow.querySelector('.inc_product_desc_att_color_block.show_dropdown').classList.remove('show_dropdown')
                }
                ...
                if (productblockmod.querySelector('.inc_product_desc_att_cloned_color_list_item_block.inc_active') != null) {
                    productblockmod.querySelector('.inc_product_desc_att_cloned_color_list_item_block.inc_active').classList.remove('inc_active')
                }
                if (productblockshow.querySelector('.inc_product_desc_att_cloned_color_list_item_block.inc_active') != null) {
                    productblockshow.querySelector('.inc_product_desc_att_cloned_color_list_item_block.inc_active').classList.remove('inc_active')
                }
                if(productblockmod.querySelector('.inc_product_desc_att_color_list_item_block .inc_product_desc_att_color_list_item_img_block[datacolor="' + colorlowertext + '"]') != null){
                    ...
                    productblockmod.querySelector('.inc_product_desc_att_cloned_color_list_item_block .inc_product_desc_att_cloned_color_list_item_img_block[datacolor="' + colorlowertext + '"]').parentNode.classList.add('inc_active')
                    productblockshow.querySelector('.inc_product_desc_att_cloned_color_list_item_block .inc_product_desc_att_cloned_color_list_item_img_block[datacolor="' + colorlowertext + '"]').parentNode.classList.add('inc_active')    
                    ...
                    if(productblockmod.querySelector('.inc_product_desc_att_cloned_color_list_item_block.inc_active .inc_product_desc_att_cloned_color_list_item_img_block img') != null){
                        let selimg = productblockmod.querySelector('.inc_product_desc_att_cloned_color_list_item_block.inc_active .inc_product_desc_att_cloned_color_list_item_img_block img').src
                        elselcolortextmodalclone.innerHTML= "<img class='varientselectedImg' src='"+selimg+"'><span>" +colorlowertext+ "</span>";
                        elselcolortextshowclone.innerHTML= "<img class='varientselectedImg' src='"+selimg+"'><span>" +colorlowertext+ "</span>";
                    }
                    if(productblockmod.querySelector('.inc_product_desc_att_color_list_item_block.inc_active div') != null){
                        ...
                        if(window.innerWidth < 821){
                            ...
                            elclonedcolortextmodal.style.backgroundColor = color_colorcode;
            ...
            if(productblockmod.querySelector('.inc_product_desc_qty_add_btn_block .inc_product_desc_pack_of_list_block.active') != null){
                productblockmod.querySelector('.inc_product_desc_qty_add_btn_block .inc_product_desc_pack_of_list_block.active').classList.remove("active");
            }
            if(productblockshow.querySelector('.inc_qty_addBtn_block .inc_mobile_product_desc_pack_of_list_block.active') != null){
                productblockshow.querySelector('.inc_qty_addBtn_block .inc_mobile_product_desc_pack_of_list_block.active').classList.remove("active");
            }
        }
    }
    ...
    INC.methods.updateProductBlock = function (productBlockel, findObj, blockType) {
        ...
        if (productBlock.querySelector('.inc_product_variant_simple_text') != null) {
            if (blockType == "sidebar" || blockType == "am") {
                productBlock.querySelector('.inc_product_variant_simple_text').innerText = "Select Shade"
                productBlock.querySelector('.inc_product_variant_simple_text').setAttribute('title', 'Select Shade')
            } else {
    ...
    INC.methods.addProductToCart = function (findObjd, productBlock, add_btn_click, _add_prod_im, btn_addtocart, newbtntext,rec_type) {
        ...
        let csrf = JSON.parse(document.querySelector('#feelunique-preferences').getAttribute('data-feelunique-preferences'));
        ...
        if (productObj.ProductType == "configurable") {
            params = addProductURL + "?" + "p=" + productObj['id'].split('-')[0] + "&quantity=" + qty + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&s=" + productObj['childsku'].split('-')[1] + "&pwa=1"
        } else {
            params = addProductURL + "?" + "p=" + productObj['id'] + "&quantity=" + qty + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&pwa=1"
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.errorMessage == "is out of stock" || data.errorMessage != "") {
                    ...
                    prod_qty_notinstock(data.errorMessage,productObj.mainId)
                }
                ...
                if(res != undefined){
                    if(res.statusText != undefined){
                        if(res.statusText == "Internal Server Error"){
                            prod_qty_notinstock(res.statusText,productObj.mainId)
                            let call_addprodtosidebar=""
        ...
        Object.keys(dataStore.bundleCartProducts).forEach(function (bundleCartProduct) {
            ...
            if (productObj.ProductType == "configurable") {
                ...
                if (count_pr_to_add == 1) {
                    url1 = addProductURL + "?" + "p=" + productObj['id'].split('-')[0] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&s=" + productObj['childsku'].split('-')[1] + "&pwa=1";
                } else if (count_pr_to_add == 2) {
                    url2 = addProductURL + "?" + "p=" + productObj['id'].split('-')[0] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&s=" + productObj['childsku'].split('-')[1] + "&pwa=1";
                } else if (count_pr_to_add == 3) {
                    url3 = addProductURL + "?" + "p=" + productObj['id'].split('-')[0] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&s=" + productObj['childsku'].split('-')[1] + "&pwa=1";
                } else if (count_pr_to_add == 4) {
                    url4 = addProductURL + "?" + "p=" + productObj['id'].split('-')[0] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&s=" + productObj['childsku'].split('-')[1] + "&pwa=1";
                } else if (count_pr_to_add == 5) {
                    url5 = addProductURL + "?" + "p=" + productObj['id'].split('-')[0] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&s=" + productObj['childsku'].split('-')[1] + "&pwa=1";
                } else if (count_pr_to_add == 6) {
                    url6 = addProductURL + "?" + "p=" + productObj['id'].split('-')[0] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken + "&s=" + productObj['childsku'].split('-')[1] + "&pwa=1";
                }

            } else {
                count_pr_to_add++
                if (count_pr_to_add == 1) {
                    url1 = addProductURL + "?" + "p=" + productObj['id'] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken
                } else if (count_pr_to_add == 2) {
                    url2 = addProductURL + "?" + "p=" + productObj['id'] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken
                } else if (count_pr_to_add == 3) {
                    url3 = addProductURL + "?" + "p=" + productObj['id'] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken
                } else if (count_pr_to_add == 4) {
                    url4 = addProductURL + "?" + "p=" + productObj['id'] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken
                } else if (count_pr_to_add == 5) {
                    url5 = addProductURL + "?" + "p=" + productObj['id'] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken
                } else if (count_pr_to_add == 6) {
                    url6 = addProductURL + "?" + "p=" + productObj['id'] + "&quantity=" + dataStore.bundleCartProducts[bundleCartProduct] + "&ajax=y" + "&e=1" + "&csrfToken=" + csrf.csrfToken
                }
            }
            if (total_length.length == count_pr_to_add) {
                ajax_add_to_cart(url1, url2, url3, url4, url5, url6, btn_cart, mainProductId,recsType)
            }
        });
        add_to_cart_tracking_multiple(mainProductId)
    }

    function ajax_add_to_cart(url1, url2, url3, url4, url5, url6, btn_cart, mainProductId,recsType) {
        INC.clientConfig.OOS = []
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.errorMessage == "is out of stock" || data.errorMessage != "" || data.errorMessage != "") {
                        prod_qty_notinstock(null, url1.split('buy?p=')[1].split("&quantity")[0])
                    }
                    suc1(btn_cart)
                }
            });
        ...
        function callprodB() {
            $.ajax({
                ...
                success: function (res) {
                    if (res.errorMessage == "is out of stock" || res.errorMessage != "" || res.errorMessage != "") {
                        prod_qty_notinstock(null, url2.split('buy?p=')[1].split("&quantity")[0])
                    }
                    suc2(btn_cart)
                }
            });
        ...
        function callprodC() {
            $.ajax({
                ...
                success: function (res2) {
                    if (res2.errorMessage == "is out of stock" || res2.errorMessage != "" || res2.errorMessage != "") {
                        prod_qty_notinstock(null, url3.split('buy?p=')[1].split("&quantity")[0])
                    }
                    suc3(btn_cart)
                }
            });
        ...
        function callprodD() {
            $.ajax({
                ...
                success: function (res3) {
                    if (res3.errorMessage == "is out of stock" || res3.errorMessage != "" || res3.errorMessage != "") {
                        prod_qty_notinstock(null, url4.split('buy?p=')[1].split("&quantity")[0])
                    }
                    suc4(btn_cart)
                }
            });
        ...
        function callprodE() {
            $.ajax({
                ...
                success: function (res4) {
                    if (res4.errorMessage == "is out of stock" || res4.errorMessage != "" || res4.errorMessage != "") {
                        prod_qty_notinstock(null, url5.split('buy?p=')[1].split("&quantity")[0])
                    }
                    suc5(btn_cart)
                }
            });
        ...
        function callprodF() {
            $.ajax({
                ...
                success: function (res5) {
                    if (res5.errorMessage == "is out of stock" || res5.errorMessage != "" || res5.errorMessage != "") {
                        prod_qty_notinstock(null, url6.split('buy?p=')[1].split("&quantity")[0])
                    }
                    loadsidebarajax(btn_cart,recsType)
                }
            });
        ...
        if (clickedElement != null) {
            try {
                if (clickedElement.className == "inc_product_modal_block inc_active") {
                    document.querySelector('body').classList.remove('inc_popup_open');
                    clickedElement.querySelector('.inc_product_desc_pack_of_list_block').classList.remove('active');
                    clickedElement.querySelector('.inc_product_desc_att_color_block').classList.remove('show_dropdown');
                    if (document.querySelector('.inc_sidebar_modal_block.inc_active') == null) {
                        document.querySelector('html').classList.remove('inc_overlay');
                ...
                if (clickedElement.className == "inc_product_desc_block" || clickedElement.className == "inc_product_desc_ecirp_block" || clickedElement.className == "inc_product_desc_att_color_list_block greater6" || clickedElement.className == "inc_product_desc_add_block Inc_checked" || clickedElement.classList.contains("inc_product_desc_ratings_block") == trueflag || clickedElement.className == 'inc_product_header_manufacturer_block' || clickedElement.className == 'inc_product_header_block' || clickedElement.className == 'inc_product_header_main_block' || clickedElement.className == 'inc_product_header_main_title_block' || clickedElement.className == 'inc_product_description_main_block inc_active' || clickedElement.className == 'productpage-whatwesay' || clickedElement.className == 'productpage-directions-content' || clickedElement.className == 'productpage-ingredients-content' || clickedElement.className == 'productpage-ingredients' || clickedElement.className == 'productpage-directions') {
                    if(clickedElement.querySelector('.inc_product_desc_pack_of_list_block') != null){
                        clickedElement.querySelector('.inc_product_desc_pack_of_list_block').classList.remove('active');
                    }
                    if(clickedElement.parentNode.querySelector('.inc_product_desc_pack_of_list_block') != null){
                        clickedElement.parentNode.querySelector('.inc_product_desc_pack_of_list_block').classList.remove('active');
                    }
                    if(clickedElement.parentNode.parentNode.querySelector('.inc_product_desc_pack_of_list_block') != null){
                        clickedElement.parentNode.parentNode.querySelector('.inc_product_desc_pack_of_list_block').classList.remove('active');
                    }
                    if(clickedElement.parentNode.parentNode.parentNode.querySelector('.inc_product_desc_pack_of_list_block') != null){
                        clickedElement.parentNode.parentNode.parentNode.querySelector('.inc_product_desc_pack_of_list_block').classList.remove('active');
                    }
                    if(clickedElement.parentNode.parentNode.parentNode.parentNode.querySelector('.inc_product_desc_pack_of_list_block') != null){
                ...
                if (document.querySelector('.inc_product_modal_block.inc_active') != null) {
                    if (clickedElement.parentNode.classList == "inc_product_desc_att_color_list_item_img" || clickedElement.classList == "inc_product_desc_att_color_list_item_img" || clickedElement.classList == "inc_product_desc_att_color_list_block greater6" || clickedElement.parentNode.classList == "inc_product_desc_att_zero_list_block" || clickedElement.classList == "inc_product_desc_att_zero_list_block" || clickedElement.classList.contains("inc_product_desc_ratings_block") == trueflag || clickedElement.className == 'inc_product_header_manufacturer_block' || clickedElement.className == 'inc_product_header_block' || clickedElement.className == 'inc_product_header_main_block' || clickedElement.className == 'inc_product_header_main_title_block' || clickedElement.className == 'inc_product_description_main_block inc_active') {
                ...
                if (INC.config.pageType == "pdp") {
                    if (clickedElement.parentNode != null){
                        if (clickedElement.parentNode.classList.contains("productpage-buy-button-row") == trueflag){
                            if (clickedElement.id == "buyProduct" || clickedElement.innerText == "Add to Bag" || (clickedElement.innerText == "ADDED" && clickedElement.parentNode.parentNode.parentNode.parentNode.querySelector('button') != null) || (clickedElement.innerText == "ADDED" && clickedElement.parentNode.parentNode.parentNode.querySelector('button') != null) || (clickedElement.innerText == "Added" && clickedElement.parentNode.parentNode.parentNode.querySelector('button') != null)) {
                                ...
                                setTimeout(function () {
                                    if (clientATCFlag == trueflag){
                                        let elPDP = document.querySelector('.productpage-details')
                                        INC.config.pdpaddedProductList = []
                ...
                if (clickedElement != null) {
                    ...
                    if (clickedElement.parentNode != null) {
                        if (clickedElement.className == "inc_product_desc_ecirp_block" || clickedElement.className == "inc_product_desc_block"){
                            if (document.querySelector('.inc_product_desc_att_block .show_dropdown') != null) {
                                document.querySelector('.inc_product_desc_att_block .show_dropdown').classList.remove('show_dropdown')
                            }
                        }  
                    ...
                    if (INC.config.pageType == "pdp") {
                        ...
                        if (clickedElement.parentNode != null) {
                            if (clickedElement.parentNode.classList != undefined) {
                                if (clickedElement.parentNode.classList.contains('option-item-swatch') == trueflag || clickedElement.parentNode.classList.contains('option-item') == trueflag) {
                                    let click_textsel = ""
                                    if(clickedElement.getAttribute('data-option') != null){
                                        click_textsel = clickedElement.getAttribute('data-option')
                                    }
                                    if (click_textsel != undefined || click_textsel != "") {
                                        click_textsel = click_textsel.trim().toLocaleLowerCase()
                                    }
                                    autoselect_(click_textsel)
                                }
                                if(clickedElement.parentNode.parentNode != null){
                                    if (clickedElement.parentNode.classList.contains('sub-products') == trueflag || clickedElement.classList.contains('swatch') == trueflag || clickedElement.parentNode.parentNode.classList.contains('sub-products') == trueflag || clickedElement.parentNode.parentNode.parentNode.classList.contains('sub-products') == trueflag) {
                                        let click_textsel = ""
                                        if(clickedElement.querySelector('.name') != null){
                                            click_textsel = clickedElement.querySelector('.name').innerText.trim();
                                        }else if(clickedElement.parentNode.querySelector('.name') != null){
                                            click_textsel = clickedElement.parentNode.querySelector('.name').innerText.trim();
    ...
    jQuery('.Product .Product-buy a').on("click", function (ev) {
        if (INC.config.pageType != "pdp") {
            document.querySelector('body').classList.add('inc_sidebar_avail')
            let thiselm = this
            let prod_wrapper = thiselm.parentNode.parentNode
            let checkmodl = setInterval(function () {
                if (document.querySelector('.simplemodal-container') != null) {
    ...
    function plp_modal_pr(prod_wrapper) {
        let prd_name = "";
        if (prod_wrapper.querySelector('.Product-summary ') != null) {
            prd_name = prod_wrapper.querySelector('.Product-summary ').innerText.trim();
        }
        let prc = prod_wrapper.querySelector('.Product-price').innerText.replace('£', '').replace('from', '').trim();
        let s_prc = null;
        let s_prc_R = null
        if (prod_wrapper.querySelector('.Product-price .Price-no-deal') != null) {
            prc = prod_wrapper.querySelector('.Product-price').innerText.replace('£', '').replace('from', '').trim();
        }else{
            prc = prod_wrapper.querySelector('.Product-price .Product-value').innerText.replace('£', '').replace('from', '').trim();
            s_prc_R = prod_wrapper.querySelector('.Product-price').innerText.replace('£', '').replace('from', '').trim();
            s_prc = s_prc_R.split('£')[0].split('/')[0].replace(' ', '').replace(',', '').trim();
        } 
        let prd_img_url = ""
        if (prod_wrapper.querySelector('.Product-image-wrapper img') != null) {
            prd_img_url = prod_wrapper.querySelector('.Product-image-wrapper img').src;
        }
        let pr_id = prod_wrapper.getAttribute('data-sku');
        let qty = 1
        let size;
        let color;
        let size_c = size + " , " + color
        if (prod_wrapper.querySelector('.quantity-wrapper .quantity-input') != null) {
            qty = prod_wrapper.querySelector('.quantity-wrapper .quantity-input').value
        }
        if (isNaN(qty)) qty = 1;
        qty = Math.abs(qty)
        if (pr_id != undefined) {
            let prod_id = pr_id;
            setTimeout(function () {
                plp_listeners(prd_name, prc, prd_img_url, s_prc, qty, prod_id, size_c);
            }, 200)
        }
    }
    ...
    function plp_listeners(prd_name_, old_prc_, prd_img_url_, prd_prc_, qty_shade_, prod_id,size_c_) {
        ...
        INC.config.plpProductId.push(productId)
